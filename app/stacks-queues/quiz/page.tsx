...
const QUIZ_QUESTIONS = [
  ...
  {
    question:
      "If you enqueue 3 items into a Queue (A, then B, then C), which item will be dequeued first?",
    options: ["C", "B", "A", "They are all dequeued at once"],
    correctAnswer: 2,
  },
  {
    question:
      "Which real-world scenario perfectly matches a Stack data structure?",
    options: [
      "A line of cars at a drive-thru",
...
export default function StacksQueuesQuizPage() {
  ...
  const nextQuestion = () => {
    if (currentQuestion < QUIZ_QUESTIONS.length - 1) {
      ...
      if (score >= 2) completeModule("stacks-queues", 50);
    }
  };
        ...
        <div className="flex justify-end">
          {!isAnswerChecked ? (
            <button
              ...
              className="px-8 py-3 bg-amber-600 hover:bg-amber-500 text-white rounded font-bold disabled:bg-slate-800 transition"
            >
              Check Answer
            </button>
          ) : (
