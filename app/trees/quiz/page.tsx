...
const QUIZ_QUESTIONS = [
  ...
  {
    question: "What do we call a node that has no children?",
    options: ["A Root", "A Branch", "A Leaf", "A Seed"],
    correctAnswer: 2,
  },
  {
    question: "Why are Binary Search Trees incredibly fast for finding data?",
    options: [
      "Because they store everything in the CPU cache",
      "Because they use arrays under the hood",
      "Because every step down the tree eliminates half of the remaining possibilities",
...
export default function TreesQuizPage() {
  ...
  const nextQuestion = () => {
    if (currentQuestion < QUIZ_QUESTIONS.length - 1) {
      ...
      if (score >= 2) completeModule("trees", 50);
    }
  };
        ...
        <div className="flex justify-end">
          {!isAnswerChecked ? (
            <button
              ...
              className="px-8 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-bold disabled:bg-slate-800 transition"
            >
              Check Answer
            </button>
          ) : (
